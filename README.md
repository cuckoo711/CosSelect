...
- **私密访问**：无公开分享，参与者需 8 位口令进入（24 小时有效，可随时重置）；空间 ID 为不可枚举的随机串
- **一键进入链接**：团长可生成「参与者链接」（自动填口令）和「团长管理链接」（含密钥）
- **成员审批**：可开关。开启时参与者设置 CN（圈名）后需团长通过/拒绝才能进入；WebSocket 实时推送申请与审批结果
- **免注册**：参与者用 CN 识别身份，相同 CN 自动恢复历史评分/批注/喜欢
- **闲置自动销毁**：团长每次进入会提示"超过 30 天未访问将自动销毁"；服务端实际超过 90 天无人访问才真正删除空间与磁盘文件（阈值可在 .env 配置）
- **团长管理**：无限级分类（折叠展开 + 拖动排序）、批量上传、**按文件夹整体导入**（拖入文件夹树自动建分类，仅收 JPG）、缩略图异步生成、统计看板、CSV 导出
...
1. **团长**：门户选「我是团长」→ 创建空间 → **保存管理密钥**（后续管理凭证）与一键链接。
2. 进入「分类与上传」：建分类、拖动排序、批量上传图片；在「成员审批」里开关审批、通过/拒绝申请。
3. **参与者**：打开团长发的参与者链接（或选「我是参与者」输入口令）→ 设置 CN → 进入。若空间开启审批，会先进入等待页，团长通过后自动进入。
